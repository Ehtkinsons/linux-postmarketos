...
static int wcd9320_parse_dt(struct wcd9320 *wcd)
{
	...
	clk_prepare_enable(wcd->codec_clk); 
...
static irqreturn_t irq_handler(int irq, void *dev_id)
{
	...
	if (status[0] & 1) {
		regmap_read(wcd->ifd_regmap,
			    TAIKO_SLIM_PGD_PORT_INT_STATUS_RX_0 + 0, &istatus[0]);
		regmap_read(wcd->ifd_regmap,
			    TAIKO_SLIM_PGD_PORT_INT_STATUS_RX_0 + 1, &istatus[1]);
		regmap_read(wcd->ifd_regmap,
			    TAIKO_SLIM_PGD_PORT_INT_STATUS_RX_0 + 2, &istatus[2]);
		regmap_read(wcd->ifd_regmap,
			    TAIKO_SLIM_PGD_PORT_INT_STATUS_RX_0 + 3, &istatus[3]);
		printk("irq2 %X %X %X %X\n",
		       istatus[0], istatus[1], istatus[2], istatus[3]);
		regmap_write(wcd->ifd_regmap,
			     TAIKO_SLIM_PGD_PORT_INT_CLR_RX_0 + 0, istatus[0]);
		regmap_write(wcd->ifd_regmap,
			     TAIKO_SLIM_PGD_PORT_INT_CLR_RX_0 + 1, istatus[1]);
		regmap_write(wcd->ifd_regmap,
			     TAIKO_SLIM_PGD_PORT_INT_CLR_RX_0 + 2, istatus[2]);
		regmap_write(wcd->ifd_regmap,
			     TAIKO_SLIM_PGD_PORT_INT_CLR_RX_0 + 3, istatus[3]);

		for (i = 0; i < 32; i++) {
			if (istatus[i / 8] & (1 << i & 7)) {
				regmap_read(wcd->ifd_regmap,
					    TAIKO_SLIM_PGD_PORT_INT_RX_SOURCE0 + i,
					    &tmp);
				printk("%u %u\n", i, tmp);
			}
		}
...
static int wcd9320_bring_up(struct wcd9320 *wcd)
{
	...
	for(i = 0; i < 3; i++){
		regmap_write(wcd->regmap, WCD9XXX_A_INTR_LEVEL0 + i, 1);
